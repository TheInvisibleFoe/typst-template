...
#let sentence-logic(body) = {
  show figure.where(kind: "example"): it => {
    ...
    grid(
      columns: (1cm, 1fr),
      column-gutter: 0.5em,
      context [(#(counter("grape-suite-sentence-counter").at(here()).first()+1))], it.body,
    )
  }
...
#let sentence(body) = {
  figure(
    kind: "example",
    supplement: context state("grape-suite-element-sentence-supplement", "Example").final(),
...
#let format-heading-numbering(body) = {
  show heading: it => context {
    ...
    pad(
      left: x-offset,
      par(hanging-indent: -1 * x-offset, text(fill: purple.lighten(25%), num) + [] + text(fill: purple, it.body)),
    )
  }
