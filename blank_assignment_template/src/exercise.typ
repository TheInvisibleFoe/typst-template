...
#let project(
  ...
  message: (
    points-sum,
    extrapoints-sum,
  ) => [In sum #points-sum + #extrapoints-sum P. are achievable. You achieved #box(line(stroke: purple, length: 1cm)) out of #points-sum points.],
  grade-scale: (
    ([excellent], 0.9),
    ([very good], 0.8),
    ([good], 0.7),
  ...
  if title == none {
    title = (
      if type != none or no != none [ #type #no ]
        + if (type != none or no != none) and suffix-title != none [ --- ]
  ...
  set page(
    ...
    background: context {
      let point-list = query(metadata).filter(e => if otype(e.value) == dictionary and "type" in e.value {
